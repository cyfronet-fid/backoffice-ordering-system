...
def _resolve_message_recipients(order: Order, scope: MessageScope, sender_email: str) -> list[User]:
    ...
    return [u for u in order.users if any(role in allowed_roles for role in u.user_type) and u.email != sender_email]
