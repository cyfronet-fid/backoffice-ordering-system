...
def test_userinfo_missing_email_raises_401(client, example_jwt, protected_oidc_path):
    with (
        ...
        patch(
            ...
            new=AsyncMock(return_value={"name": "Jane Doe"}),  # no email
        ),
    ):
        mock_jwks_client.get_signing_key_from_jwt.return_value = MagicMock()
...
def test_userinfo_network_failure_raises_401(client, example_jwt, protected_oidc_path):
    with (
        ...
        patch(
            "backend.auth._fetch_userinfo",
            new=AsyncMock(return_value={}),  # network failure returns empty dict
        ),
    ):
        mock_jwks_client.get_signing_key_from_jwt.return_value = MagicMock()
        mock_jwt_decode.return_value = {"sub": "some-sub"}  # no name/email in JWT
...
def test_userinfo_token_rejected_raises_401(client, example_jwt, protected_oidc_path):
    with (
        ...
        patch(
            "backend.auth._fetch_userinfo",
            new=AsyncMock(side_effect=HTTPException(status_code=401, detail="Token rejected by UserInfo endpoint")),
        ),
    ):
        mock_jwks_client.get_signing_key_from_jwt.return_value = MagicMock()
        mock_jwt_decode.return_value = {"sub": "some-sub"}  # no name/email in JWT
...
def test_userinfo_cached_by_sub():
    from backend.auth import _fetch_userinfo, _userinfo_cache
    ...
    with patch("backend.auth._http_client.get", new=AsyncMock(return_value=mock_response)) as mock_get:
        result1 = asyncio.run(_fetch_userinfo("Bearer token-v1", "sub-abc"))
        result2 = asyncio.run(_fetch_userinfo("Bearer token-v2", "sub-abc"))  # refreshed token, same sub
